...
get_ue_rnti_cellid(Ptr<ns3::NetDevice> ueNetDevice)
{
...
void
ReportUeSinrRsrp(uint16_t cellId,
                 uint16_t rnti,
                 double rsrp,
                 double sinr,
...
void
RxCallback(const std::string path, Ptr<const Packet> packet, const Address& from)
{
...
void
sendStream(Ptr<Node> sendingNode, Ptr<Node> receivingNode, int size)
{
...
std::string
extractModelPath(const std::string& input)
{
